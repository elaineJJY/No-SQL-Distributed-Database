...
在分布式系统中，引导服务器（bootstrap server）是指用于引导（初始化）新加入节点或者删除需要下线的节点的特殊服务器。它作为管理哈希环上各个节点的中心节点，有着重要的作用：包括更新哈希环，引导相关节点进行数据转移，监控各节点的状态等。通过引导服务器的存在和功能，分布式系统能够更加稳定和可靠地管理节点的加入和退出过程，同时优化数据的分布和访问效率，提供高性能和可扩展性的系统架构。

### **功能**

* 添加KVServer 当一个新的KVServer连接到ECS时，ECS会执行以下操作：

  * 通过KVServer用于与客户端通信的服务器端口地址，确定新存储服务器在环中的位置。
  * 重新计算并更新哈希环。
  * 使用更新后的哈希环初始化新的存储服务器。
  * 在后继节点上设置写锁。
  * 数据转移：调用受影响数据项的传输到新的存储服务器。
  * 当所有受影响的数据已传输完成，向所有存储服务器发送更新后的哈希环。

* 移除KVServer 当一个KVServer像ECS发送下线通知，或者ECS检测到某节点故障，ECS会执行以下操作：

  * ECS会将该节点加入待删除队列，并逐一处理。
  * 重新计算并更新哈希环。
  * 数据转移：调用受影响数据项的传输到新的存储服务器。
  * 当所有受影响的数据已传输完成，ECS向剩余的存储服务器发送更新后的哈希环。
  * 继续关闭存储服务器。

* 监控KVServer的状态

  ECS会分一个单独的线程来轮训监测服务器是否下线，具体来说当 `socket.isclose() == ture` 的时候就是下线了，此时会调用下线的逻辑
